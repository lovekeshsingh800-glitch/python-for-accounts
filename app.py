...
# =========================================================================
# --- 📅 NEW MENU: MONTHLY EXPENSE INSIGHTS ENGINE (BY LOVEKESH) ---
# =========================================================================

# Check if the monthly option code is active or clicked
if 'selected_tab' in locals() and "Monthly" in str(selected_tab):
    st.title("📅 Monthly Expense Insights")
    st.markdown("Yahan aap mahine ke hisaab se apna total expense track kar sakte hain.")
    st.markdown("---")
    
    if 'filtered_df' in locals() and not filtered_df.empty:
        # Data copy karke month-year format banana
        monthly_df = filtered_df.copy()
        monthly_df['Month-Year'] = monthly_df['Date'].dt.strftime('%B %Y')
        
        # Month-wise total kharcha calculate karna
        monthly_summary = monthly_df.groupby('Month-Year')['Amount Spent (₹)'].sum().reset_index()
        
        # Display as a Beautiful Data Cards (Metrics)
        st.subheader("💳 Month-Wise Summary")
        
        # Columns create karna blocks dikhane ke liye
        cols = st.columns(len(monthly_summary) if len(monthly_summary) > 0 else 1)
        for idx, row in monthly_summary.iterrows():
            with cols[idx % len(cols)]:
                st.metric(
                    label=f"Total Spent ({row['Month-Year']})", 
                    value=f"₹{row['Amount Spent (₹)']:,.2f}"
                )
            
        st.markdown("---")
        
        # Refined Bar Chart for Monthly Comparison
        ...
        # Dark Theme layout matching for Plotly
        fig_month.update_layout(
            plot_bgcolor='rgba(20, 23, 34, 0.5)',
            paper_bgcolor='rgba(0,0,0,0)',
            font=dict(color='#ffffff'),
            xaxis=dict(gridcolor='rgba(255, 255, 255, 0.05)', title="Months"),
            yaxis=dict(gridcolor='rgba(255, 255, 255, 0.05)', tickprefix="₹", title="Total Amount")
        )
